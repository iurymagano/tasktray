...
ipcMain.handle('set-tasks', async (event, tasks) => {
  console.log('[IPC] set-tasks chamado com:', tasks);
  store.set('tasks', tasks);
  console.log('[electron-store] Conteúdo salvo:', store.get('tasks'));
  return true;
});
...
app.whenReady().then(() => {
  ...
  const ok = globalShortcut.register('CommandOrControl+K', () => {
    if (!win) return;
    if (win.isVisible()) {
      win.hide();
    } else {
  ...
  if (ok) {
    console.log('[Electron] Atalho Ctrl+K registrado com sucesso');
  } else {
    console.error(
      '[Electron] Falha ao registrar atalho Ctrl+K. Pode estar em uso por outro app ou bloqueado pelo sistema.',
    );
  }

  app.on('will-quit', () => {
    globalShortcut.unregisterAll();
    console.log('[Electron] Atalhos liberados');
  });
